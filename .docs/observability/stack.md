...
- `scripts/stack-up.sh` auto-discovers the common local binary names for VictoriaLogs / VictoriaMetrics (`victoria-logs`, `victoria-metrics`) and prefers the repo-local `.observability/bin/victoria-traces-prod` for traces when no explicit overrides are set.
...
- `scripts/stack-up.sh` uses `nohup` + PID files for local backend lifecycle on all platforms; `stack-down.sh` still understands legacy `launchctl:` handles so older stack instances can be cleaned up.
